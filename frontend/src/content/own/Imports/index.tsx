...
export interface OwnHeader {
  label: string;
  englishLabel?: string; // English label for CSV matching, independent of user language
  keyName: ImportKeys;
  required?: boolean;
...
const Import = ({ }: OwnProps) => {
  ...
  // Enhanced matching logic: try English labels first, then translated labels
  useEffect(() => {
    if (userHeaders.length) {
      let result: { userHeader: string; keyName: string }[] = [];
      headerKeysConfig[entity].forEach((ownHeader) => {
        // Priority 1: Try matching with English label if available
        const labelToMatch = ownHeader.englishLabel || ownHeader.label;
        ...
        if (
          closestUserHeader &&
          distance(closestUserHeader as string, labelToMatch) < 5 &&
