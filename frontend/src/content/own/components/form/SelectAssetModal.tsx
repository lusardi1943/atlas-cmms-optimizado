...
const SelectAssetModal: React.FC<SelectAssetModalProps> = ({
  ...
  const filteredAssetsHierarchy = useMemo(() => {
    const directMatches = assetsHierarchy.filter(
      (asset) =>
        !excludedAssetIds.includes(asset.id) &&
        (locationId ? asset.locationId === locationId : true)
    );
    const allIdsToShow = new Set<number>();
    directMatches.forEach((asset) => {
      asset.hierarchy.forEach((id) => allIdsToShow.add(id));
    });
    return assetsHierarchy.filter((asset) => allIdsToShow.has(asset.id));
  }, [assetsHierarchy, locationId, excludedAssetIds]);
