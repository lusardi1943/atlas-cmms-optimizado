...
import SelectAssetTree from './SelectAssetTree';
...
export const CustomSelect = ({
  ...
  switch (field.type2) {
    ...
    case 'location':
    case 'parentLocation': {
      const allParentIds = new Set(locationsMini.map((l) => l.parentId));
      options = locationsMini
        .filter(
          (location) =>
            location.id !== excluded &&
            (!field.leafOnly || !allParentIds.has(location.id))
        )
        .map((location) => {
          return {
            label: location.name,
    ...
    case 'asset': {
      ...
      return (
        <>
          <SelectAssetTree
            assets={assetsMini.filter((asset) => !locationId || asset.locationId === locationId)}
            value={field.multiple ? fieldValue ?? [] : fieldValue ?? null}
            onSelect={(newValue) => {
              handleChange(formik, field.name, newValue);
            }}
