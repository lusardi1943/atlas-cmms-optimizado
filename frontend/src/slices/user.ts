...
export const disableUser =
  (id: number, deactivatedUntil?: string): AppThunk =>
    async (dispatch) => {
      await api.patch<UserResponseDTO>(`${basePath}/${id}/disable`, {
        deactivatedUntil
      });
...
export const softDeleteUser =
  (id: number): AppThunk =>
    async (dispatch) => {
      await api.patch<UserResponseDTO>(`${basePath}/soft-delete/${id}`, {});
      dispatch(slice.actions.deleteUser({ id }));
    };
...
export const enableUser =
  (id: number): AppThunk =>
    async (dispatch) => {
      await api.patch<UserResponseDTO>(`${basePath}/${id}/enable`, {});
      dispatch(slice.actions.deleteUser({ id }));
    };
