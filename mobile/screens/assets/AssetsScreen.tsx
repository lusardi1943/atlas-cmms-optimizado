...
export default function AssetsScreen({
  ...
  const [criteria, setCriteria] = useState<SearchCriteria>(
    getCriteriaFromFilterFields(route.params?.filterFields ?? [])
  );
  useEffect(() => {
  ...
  // Reset search when screen is focused (entering from root)
  useFocusEffect(
    React.useCallback(() => {
      // Only reset if we're at the root (no parent id)
      if (!route.params?.id) {
        setSearchQuery('');
  ...
  const onQueryChange = (query) => {
    onSearchQueryChange<AssetDTO>(
      ...
      setCriteria,
      setSearchQuery,
      ['name', 'location.name', 'model', 'description', 'additionalInfos']
    );
    setView('list');
  };
  useDebouncedEffect(
  ...
  const handleViewChildren = (asset) => {
    navigation.push('Assets', {
      id: asset.id,
      hierarchy: asset.hierarchy ?? [asset.id]
    });
  };
