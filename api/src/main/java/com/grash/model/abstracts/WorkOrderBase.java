...
public abstract class WorkOrderBase extends CompanyAudit {
    ...
    // Impacto: La imagen de firma no se carga automáticamente, reduciendo el tráfico de red y memoria.
    @OneToOne(fetch = FetchType.LAZY)
    @Audited(targetAuditMode = RelationTargetAuditMode.NOT_AUDITED, withModifiedFlag = true)
    private File image;

    // [MODIFICADO] Cambiado a FetchType.LAZY.
    // Impacto: Evita cargar la categoría completa en listados de órdenes de trabajo.
    @ManyToOne(fetch = FetchType.LAZY)
    @Audited(targetAuditMode = RelationTargetAuditMode.NOT_AUDITED, withModifiedFlag = true)
    private WorkOrderCategory category;

    // [MODIFICADO] Cambiado a FetchType.LAZY.
    // Impacto: Optimiza consultas al no cargar datos de ubicación (que pueden ser pesados) por defecto.
    @ManyToOne(fetch = FetchType.LAZY)
    @Audited(targetAuditMode = RelationTargetAuditMode.NOT_AUDITED, withModifiedFlag = true)
    private Location location;

    // [MODIFICADO] Cambiado a FetchType.LAZY.
    // Impacto: Mejora significativa en rendimiento al evitar cargar todos los datos del equipo asociado.
    @ManyToOne(fetch = FetchType.LAZY)
    @Audited(targetAuditMode = RelationTargetAuditMode.NOT_AUDITED, withModifiedFlag = true)
    private Team team;

    // [MODIFICADO] Cambiado a FetchType.LAZY.
    // Impacto: Evita cargar detalles del usuario principal en cada consulta de orden de trabajo.
    @ManyToOne(fetch = FetchType.LAZY)
    @Audited(targetAuditMode = RelationTargetAuditMode.NOT_AUDITED, withModifiedFlag = true)
    ...
    // [MODIFICADO] Cambiado a FetchType.LAZY.
    // Impacto: Crítico para el rendimiento, ya que los activos suelen tener muchas relaciones. Evita el efecto dominó de carga.
    @ManyToOne(fetch = FetchType.LAZY)
    @Audited(targetAuditMode = RelationTargetAuditMode.NOT_AUDITED, withModifiedFlag = true)
    private Asset asset;
