...
public class UserController {
    ...
    public UserResponseDTO disable(@ApiParam("id") @PathVariable("id") Long id,
                                   @RequestBody(required = false) DeactivationRequestDTO deactivationRequestDTO,
                                   @ApiIgnore @CurrentUser OwnUser requester) {
        ...
        if (optionalUserToDisable.isPresent()) {
            ...
            if (requester.getRole().getEditOtherPermissions().contains(PermissionEntity.PEOPLE_AND_TEAMS)) {
                ...
                if (deactivationRequestDTO != null) {
                    userToDisable.setDeactivatedUntil(deactivationRequestDTO.getDeactivatedUntil());
                }
    ...
    @ApiResponses(value = {//
            ...
            @ApiResponse(code = 403, message = "Access denied"), //
            @ApiResponse(code = 404, message = "User not found")})
    public UserResponseDTO enable(@ApiParam("id") @PathVariable("id") Long id,
                                  @ApiIgnore @CurrentUser OwnUser requester) {
        Optional<OwnUser> optionalUserToEnable = userService.findByIdAndCompany(id, requester.getCompany().getId());

        if (optionalUserToEnable.isPresent()) {
            OwnUser userToEnable = optionalUserToEnable.get();
            if (requester.getRole().getEditOtherPermissions().contains(PermissionEntity.PEOPLE_AND_TEAMS)) {
                userToEnable.setEnabled(true);
                userToEnable.setEnabledInSubscription(true);
                userToEnable.setDeactivatedUntil(null);
                return userMapper.toResponseDto(userService.save(userToEnable));
            } else {
                throw new CustomException("You don't have permission", HttpStatus.NOT_ACCEPTABLE);
            }
        } else {
