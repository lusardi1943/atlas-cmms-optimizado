...
public class UserService {
    ...
    public SignupSuccessResponse<OwnUser> signup(UserSignupRequest userReq) {
        ...
        boolean isAdmin = auth != null && auth.isAuthenticated() && !auth.getPrincipal().equals("anonymousUser") &&
                auth.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_CLIENT") || a.getAuthority().equals("ROLE_SUPER_ADMIN"));
        ...
        if (!isAdmin && allowedOrganizationAdmins != null && userReq.getRole() == null && allowedOrganizationAdmins.length != 0 && Arrays.stream(allowedOrganizationAdmins).noneMatch(allowedOrganizationAdmin -> allowedOrganizationAdmin.equalsIgnoreCase(userReq.getEmail()))) {
        ...
        if (user.getRole() == null) {
            ...
            if (enableInvitationViaEmail) {
                if (!isAdmin && userInvitationService.findByRoleAndEmail(optionalRole.get().getId(),
            ...
            if (user.getCompany() == null) {
                // If bypassed or logic fell through, ensure company is set from role if possible
                if (optionalRole.get().getCompanySettings() != null) {
                    user.setCompany(optionalRole.get().getCompanySettings().getCompany());
                } else {
                    if (enableInvitationViaEmail) {
                        // Fallback for invitation flow if not handled above
                        List<UserInvitation> userInvitations = userInvitationService.findByRoleAndEmail(optionalRole.get().getId(), user.getEmail());
                        if (!userInvitations.isEmpty()) {
                            Optional<OwnUser> optionalInviter = findById(userInvitations.get(0).getCreatedBy());
                            if (optionalInviter.isPresent()) user.setCompany(optionalInviter.get().getCompany());
                        }
                    }

                    // If still null, check if creator is an Admin (authenticated user)
                    // This handles the case where an Admin creates a user directly without an invitation
                    if (user.getCompany() == null && isAdmin) {
                        Optional<OwnUser> adminUser = userRepository.findByEmailIgnoreCase(auth.getName());
                        if (adminUser.isPresent()) {
                            user.setCompany(adminUser.get().getCompany());
                        }
                    }
                }
            }
        ...
        if (Helper.isLocalhost(PUBLIC_API_URL)) {
            ...
            if (isAdmin) {
                ...
                userRepository.save(user);
                onCompanyAndUserCreation(user);
                return new SignupSuccessResponse<>(true, "User created successfully", user);
            }
    ...
    public OwnUser update(Long id, UserPatchDTO userReq) {
        if (userRepository.existsById(id)) {
            ...
            if (userReq.getEmail() != null && !userReq.getEmail().equalsIgnoreCase(savedUser.getEmail())) {
                ...
                savedUser.setEmail(userReq.getEmail().toLowerCase());
            }
